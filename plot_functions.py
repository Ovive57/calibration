...
def plot_lm(model, dm,z=[0,1,2]):
    ...
    for iz in z:
        ...
        if iz==0:
            ...
            fileobs='data/smf/henriques_2014_z0_cha.txt'
            ...
            # Observations Baldry
            fileobs = 'data/smf/baldry_2012_z0_cha.txt'
            ...
            fileobs = 'data/smf/moustakas_z0.01_z0.20.smf'
            mass_low = np.loadtxt(fileobs, skiprows=5, usecols=(0), unpack=True) - np.log10(h)
            mass_high = np.loadtxt(fileobs, skiprows=5, usecols=(1), unpack=True) - np.log10(h)
            ...
            # Observations Ilbert
            fileobs = 'data/smf/muzzin_ilbert_z0.2_z0.5.smf'
        ...
        if iz==1:
            # Observations Moustakas
            fileobs = 'data/smf/moustakas_z0.80_z1.00.smf'
            mass_low = np.loadtxt(fileobs, skiprows=4, usecols=(0), unpack=True) - np.log10(h)
            mass_high = np.loadtxt(fileobs, skiprows=4, usecols=(1), unpack=True) - np.log10(h)
            ...
            # Observations Ilbert
            fileobs = 'data/smf/muzzin_ilbert_z0.5_z1.1.smf'
            mass_low = np.loadtxt(fileobs, skiprows=4, usecols=(0), unpack=True) - np.log10(h)
            mass_high = np.loadtxt(fileobs, skiprows=4, usecols=(1), unpack=True) - np.log10(h)
        ...
        if iz==2:
            # Observations Henriques
            fileobs='data/smf/henriques_2014_z2_cha.txt'
            mass_low = np.loadtxt(fileobs, skiprows=5, usecols=(0), unpack=True) - np.log10(h)
            mass_high = np.loadtxt(fileobs, skiprows=5, usecols=(1), unpack=True) - np.log10(h)
            ...
            # Observations Ilbert
            fileobs = 'data/smf/muzzin_ilbert_z2.0_z2.5.smf'
            mass_low = np.loadtxt(fileobs, skiprows=4, usecols=(0), unpack=True) - np.log10(h)
            mass_high = np.loadtxt(fileobs, skiprows=4, usecols=(1), unpack=True) - np.log10(h)
        ...
        plt.legend()
        plt.savefig('/home/olivia/galform_calib/plots/lm_z'+ str(iz)+'.pdf')
        plt.show()
...
def plot_sfr(model, dsfr,z=[0,1,2]):
    ...
    for iz in z:
        ...
        if iz==0:
            fileobs='data/sfrf/gruppioni_2015_z0.0-0.3_cha.txt'

        if iz==1:
            fileobs='data/sfrf/gruppioni_2015_z0.8-1.0_cha.txt'

        if iz==2:
            fileobs='data/sfrf/gruppioni_2015_z2.0-2.5_cha.txt'
        ...
        plt.xlim(-2,5)
        plt.legend()
        plt.savefig('/home/olivia/galform_calib/plots/sfr_z'+ str(iz)+'.pdf')
        plt.show()
...
def plot_BHSM(model, dbulge, dbh, z=0):
    ...
    for mod in model:
        ...
        plt.ylabel('log$_{10}$ (M$_{BH}$/h$^{-1}$M$_{\\odot}$)')
        plt.xlabel('log$_{10}$ (M$_{bulge}$/h$^{-1}$M$_{\\odot}$)')

        ### OBSERVATIONS ###
        # McConnel
        fileobs='data/bhsm/McConnell_Ma_2013_ascii.txt'
        ...
        plt.xlim(8,13)
        plt.legend()
        plt.savefig('/home/olivia/galform_calib/plots/BHSM_z0.pdf')
        plt.show()

plot_BHSM(['shark'], 0.25, 0.25)
exit()
def plot_sizes(model, z=[0]):
    ...
    for iz in z:
        ...
        fig1.legend()
        fig2.legend()
        fig1.savefig('/home/olivia/galform_calib/plots/sizes_bulge_z'+ str(iz)+'.pdf')
        fig2.savefig('/home/olivia/galform_calib/plots/sizes_disk_z'+ str(iz)+'.pdf')
        plt.show()
...
def plot_himf(model, dgas,z=[0]):
    ...
    for iz in z:
        ...
        # Not yet
        plt.legend()
        plt.savefig('/home/olivia/galform_calib/plots/himf_z'+ str(iz)+'.pdf')
        plt.show()
